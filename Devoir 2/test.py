...
def test_precision_and_time():
    precision_LU = []
    precision_np = []
    order = ['statique', 'stationnaire', 'harmonique', 'dynamique']

    A, b, nodes, sol, cond = ndtfun(0.2, 1, 0, 0, 100.)
    precision_LU.append(np.linalg.norm(np.dot(A, sol) - b)/np.linalg.norm(b))
    x = np.linalg.solve(A, b)
    precision_np.append(np.linalg.norm(np.dot(A, x) - b) / np.linalg.norm(b))

    A, b, nodes, sol, cond = ndtfun(0.2, 1, 0, 100, 100.)
    precision_LU.append(np.linalg.norm(np.dot(A, sol) - b) / np.linalg.norm(b))
    x = np.linalg.solve(A, b)
    precision_np.append(np.linalg.norm(np.dot(A, x) - b) / np.linalg.norm(b))

    A, b, nodes, sol, cond = ndtfun(0.2, 1, 30, 0, 100.)
    precision_LU.append(np.linalg.norm(np.dot(A, sol) - b) / np.linalg.norm(b))
    x = np.linalg.solve(A, b)
    precision_np.append(np.linalg.norm(np.dot(A, x) - b) / np.linalg.norm(b))

    A, b, nodes, sol, cond = ndtfun(0.2, 1, 30, 100, 100.)
    precision_LU.append(np.linalg.norm(np.dot(A, sol) - b) / np.linalg.norm(b))
    x = np.linalg.solve(A, b)
    precision_np.append(np.linalg.norm(np.dot(A, x) - b) / np.linalg.norm(b))
...
# test_precision_and_time()
ndtfun(0.2, 1, 0, 0, 100)
