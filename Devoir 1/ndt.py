...
def solve():
    ...
    if (not GRAPHS):
        ...
        svdres = svd(A)
        sigma1.append(min(svdres))
        sigmaN.append(max(svdres))
...
if GRAPHS:
    freq = 0
    vel = 0
    vec = np.linspace(1e-3*cm, 0.05*cm, 200)
    for largeur in vec:
        gap = largeur
    ...
    plt.title('Etude des σ_1, σ_N et κ de la matrice A en fonction de la largeur de l\'entrefer')
    ...
    plt.title('Nombre de conditionnements κ')
    plt.xlabel('Largeur de l\'entrefer [cm]')
    plt.ylabel('κ [/]')
    ...
    plt.figure(figsize=(5,6))
    plt.title('Etude des σ_1, σ_N et κ de la matrice A en fonction de la perméabilité relative du noyau magnétique')
